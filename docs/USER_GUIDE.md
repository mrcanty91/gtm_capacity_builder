...
**Load the demo / start over.** Dashboard → "↺ Load demo data" swaps in the sample plan (banner and all); "○ Start blank" returns you to a clean model. A fresh install opens blank. Neither touches saved runs or settings — for that, Agents → **🗑 Wipe all local data** deletes everything this tool stores in the browser (model, ledger, saved runs, settings including API keys). No undo; export first.
